...
def vae_loss(x, x_pred, mean, logvar, beta=1.):
    recon_loss = torch.mean((x - x_pred)**2)
    KLD = - 0.5 * torch.mean(1 + logvar - mean**2 - torch.exp(logvar))
...
class VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        mean, logvar = self.mean(enc_out), self.logvar(enc_out)
    ...
    def fit(
        ...
        criterion=vae_loss, 
        epochs=200, 
        model_path='best_models/vae.pth',
        patience=20,  
        verbose=True,
    ):
        self.to(self.device)
        ...
        for epoch in range(epochs):
            ...
            for x_batch in loader_data:
                ...
                optimizer.zero_grad()  
                x_pred, mean, logvar = self.forward(x_batch)  
                loss, recon, kld = criterion(x_pred, x_batch, mean, logvar)  
                loss.backward() 
                optimizer.step() 
            ...
            if verbose:
                print(f'Epoch {epoch+1}/{epochs} | Loss: {loss_epoch:.10f} | Recon: {recon:.7f} | KLD: {kld:.7f}')
